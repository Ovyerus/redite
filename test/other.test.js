...
describe('Extra coverage', () => {
    ...
    describe('setStack', () => {
        it('should throw an error when not given a stack', async () => {
            await expect(wrapper.setStack(TestHash)).to.be.rejectedWith(Error, 'At least one key is required in the stack');
        });
    ...
    describe('arrayStack', () => {
        it('should throw an error on an unsupported method', () => {
            expect(() => wrapper.arrayStack('foo')).to.throw(Error, 'Method "foo" is not supported');
        });

        it('should throw an error on an empty stack', () => {
            expect(() => wrapper.arrayStack('forEach')).to.throw(Error, 'At least one key is required in the stack');
        });

        it('should throw an error when trying to call a non-mutating method on a native list', async () => {
            await expect(wrapper.arrayStack('forEach', ['foo'])()).to.be.rejectedWith(Error, 'Unable to apply method "forEach" on a first level value.');
        });

        it('should throw an error when trying to apply a method on a non-array', async () => {
            await wrapper.test.foo.set(TestHash);
            await expect(wrapper.arrayStack('map', ['test', 'foo'])()).to.be.rejectedWith(Error, `Unable to apply method "map" to a non-array (${typeof TestHash})`);
        });
    });
